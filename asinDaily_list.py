...
# ================= 2. 获取店铺 ID =================
async def get_target_shop_sid(target_name="松立-US"):
    app_id = get_appid()
...
# ================= 4. 清理旧文件模块 (放入回收站) =================
def clean_old_reports_to_trash(prefix="松立-US_ASIN统计_"):
    """在生成新文件前，将旧的统计 Excel 移至回收站"""
...
async def main():
    ...
    if sid:
        ...
        if final_list:
            ...
            file_name = f"{shop_name}_ASIN统计_{now_str}.xlsx"

            # 保存新文件前清理旧文件
            clean_old_reports_to_trash()
